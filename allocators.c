...
extern Allocator_Result _mmap_allocator_proc(
  rawptr               data,
  Allocator_Mode       mode,
  isize                size,
  isize                align,
  ...
  case AM_Alloc:
    ...
    result.value = os_pages_allocate((size + OS_PAGE_SIZE - 1) / OS_PAGE_SIZE);
    if (!result.value) {
    ...
    return result;
  case AM_Free:
    os_pages_deallocate(old_memory, (size + OS_PAGE_SIZE - 1) / OS_PAGE_SIZE);
    result.err = AE_None;
    return result;
  case AM_Free_All:
...
extern Allocator_Result arena_allocator_proc(
  rawptr               data,
  Allocator_Mode       mode,
  isize                size,
  isize                align,
...
extern void arena_allocator_destroy(Arena_Allocator arena,
                                      Allocator allocator) {
...
extern Allocator_Result pool_allocator_proc(
  rawptr               data,
  Allocator_Mode       mode,
  isize                size,
  isize                align,
...
extern Allocator_Result tracking_allocator_proc(
  rawptr data,
  Allocator_Mode mode,
  isize size,
  isize align,
...
extern Allocator tracking_allocator_init(Tracking_Allocator *track,
                                           Allocator backing_allocator) {
...
extern Allocator_Result ring_buffer_allocator_proc(
  rawptr               data,
  Allocator_Mode       mode,
  isize                size,
  isize                align,
...
extern Allocator_Result nil_allocator_proc(
  rawptr               data,
  Allocator_Mode       mode,
  isize                size,
  isize                align,
...
extern Allocator_Result panic_allocator_proc(
  rawptr               data,
  Allocator_Mode       mode,
  isize                size,
  isize                align,
...
extern Allocator_Result default_allocator_proc(
  rawptr data,
  Allocator_Mode mode,
  isize size,
  isize align,
    ...
    case 4:
      if (!da->fixed_allocator_free[size_category]) {
        da->fixed_allocator_free[size_category] =
          pool_allocator_chunk_init(os_pages_allocate(1), OS_PAGE_SIZE, 8 << size_category);
      }
...
extern void default_allocator_destroy(Default_Allocator *a) {
  (void)a;
  unimplemented();
}
